...
def submitForm(formWid, address, collectWid, schoolTaskWid, form, session, host):
    ...
    params = {"formWid": formWid,
              ...
              'uaIsCpadaily': True}
    submitForm_ = 'https://{host}/wec-counselor-collector-apps/stu/collector/submitForm'.format(
        host=host)
    r = session.post(url=submitForm_, headers=headers,
                     data=json.dumps(params), verify=not debug)
    msg = r.json()['message']
    return msg


title_text = '今日校园提交结果通知'
